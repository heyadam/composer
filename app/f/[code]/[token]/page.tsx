...
export default function FlowPage({ params }: FlowPageProps) {
  ...
  useEffect(() => {
    async function fetchFlow() {
      // Wait for auth to finish loading before determining ownership
      ...
      if (result.flow.live_id !== code) {
        router.replace(`/f/${result.flow.live_id}/${token}`);
        return;
      }

      // Determine ownership by comparing user IDs
      const ownershipStatus = user?.id === result.flow.user_id;
    ...
    fetchFlow();
  }, [code, token, router, user, authLoading]);
